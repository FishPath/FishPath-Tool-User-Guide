...
After creating an account (new user) or logging in (existing user), the user is directed to the FishPath Tool Dashboard (\ref{fig:dashboard}), or the user's ``homepage'' of the FishPath Tool. On the FishPath Tool Dashboard, users view 4 headings:
...
When a user clicks the ``FishPath Tool'' menu in the top left corner, a drop-down display allows the user to navigate to various options (\ref{fig:dashboard}). The ``Reference Materials'' tab displays all of the options contained in the FishPath Tool.
...
Next, the invoked criteria and caveats are summarized by (\ref{fig:opt-desc})
